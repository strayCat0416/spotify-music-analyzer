...
class CreateArtistAnalysisService:
    ...
    def create_artist_analysis_data(self, artist_id):
        ...
        try:
            ...
            # リリース日降順でソート
            artist_analysis_df = artist_analysis_df.sort_values(
            ...
            for i, row in artist_analysis_df.iterrows():
                ...
                chart_audio_features_df = chart_audio_features_df._append(
                    audio_features_df
                )

            artist_analysis_df = pd.merge(
                artist_analysis_df, chart_audio_features_df, on="id"
            )

            # データの整形
            artist_analysis_df["key"] = artist_analysis_df["key"].map(
                Const.PITCH_CLASS
            ) + artist_analysis_df["mode"].map(Const.MODE_DEC)
            artist_analysis_df["tempo"] = (
                artist_analysis_df["tempo"].astype(int).astype(str) + "bpm"
            )
            artist_analysis_df["duration_ms"] = (
                (artist_analysis_df["duration_ms"] // 1000 // 60)
                .astype(int)
                .astype(str)
                + ":"
                + ((artist_analysis_df["duration_ms"] // 1000) % 60)
            ...
            artist_analysis_df["time_signature"] = (
                artist_analysis_df["time_signature"].astype(int).astype(str) + "/4拍子"
            )
            artist_analysis_df["danceability"] = (
                artist_analysis_df["danceability"] * 100
            ).astype(int).astype(str) + "%"
            artist_analysis_df["energy"] = (artist_analysis_df["energy"] * 100).astype(
                int
            ).astype(str) + "%"
            artist_analysis_df["valence"] = (
                artist_analysis_df["valence"] * 100
            ).astype(int).astype(str) + "%"
            artist_analysis_df["popularity"] = (
                artist_analysis_df["popularity"].astype(int).astype(str) + "%"
            )
            artist_analysis_df["release_date"] = pd.to_datetime(
                artist_analysis_df["release_date"]
            ).dt.strftime("%Y年%m月%d日")
            artist_analysis_df["acousticness"] = (
                artist_analysis_df["acousticness"] * 100
            ).astype(int).astype(str) + "%"
            artist_analysis_df["speechiness"] = (
                artist_analysis_df["speechiness"] * 100
            ).astype(int).astype(str) + "%"
            artist_analysis_df["loudness"] = (
                artist_analysis_df["loudness"].astype(int).astype(str) + "dB"
            )
            artist_analysis_df = artist_analysis_df.rename(
                columns=Const.ARTIST_ANALYSIS_COLUMN_NAME_MAPPING
            )
            artist_analysis_df = artist_analysis_df.drop(
                columns=Const.ARTIST_ANALYSIS_DROP_LIST
            ).reindex(columns=Const.ARTIST_ANALYSIS_COLUMN_ORDER)

            # 楽曲名とアーティスト名が全く重複している場合は収録されているアルバムが違うだけで同じ曲なので、片方を削除
            artist_analysis_df = artist_analysis_df.drop_duplicates(
                subset=["アーティスト名", "楽曲名"], keep="first"
            )
        ...
        try:
            ...
            while True:
                albums = self.spotify.artist_albums(
                    artist_id, album_type=album_type, limit=limit, offset=offset
                )["items"]
